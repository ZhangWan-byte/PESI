...
def sabdab_train(config, result_path):

    # model name
    config = prepare_mlm(config=config)

    print("training on SAbDab_MLM starting...")

    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

    train_dataset = SAbDabDataset_MLM(corpus_path=config["sabdab_path"], vocab=vocab, train_test='train')

    test_dataset = SAbDabDataset_MLM(corpus_path=config["sabdab_path"], vocab=vocab, train_test='test')
...
if __name__=='__main__':
    ...
    config = {
        # data type
        "sabdab_path": "../../Transformer4Ab/data/data_list.pkl",     # data path for sabdab
    ...
    result_path = "./results/SAbDab_MLM/{}".format(config["current_time"])
    ...
    # training
    sabdab_train(config=config, result_path=result_path)